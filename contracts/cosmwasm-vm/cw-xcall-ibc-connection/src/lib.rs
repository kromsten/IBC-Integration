...
use state::{HOST_SEND_MESSAGE_REPLY_ID, IBC_ACKS, IBC_ACK_COUNTER, IBC_RES, IBC_RES_COUNTER, IBC_TOUTS, IBC_TOUT_COUNTER, REPLIES, REPLY_COUNTER};
use thiserror::Error;
...
pub fn reply(deps: DepsMut, env: Env, msg: Reply) -> Result<Response, ContractError> {
    ...
    if msg.id != HOST_SEND_MESSAGE_REPLY_ID {
        let c = REPLY_COUNTER.load(deps.storage).unwrap_or(0);
